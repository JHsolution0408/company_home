...
- Node build pipeline (`gatsby-node.js`):
  ...
  - Template used: `src/templates/markdown-page.js` (ensure it exists for new content types).
...
- When adding a new content area:
  ...
  - Update any templates under `src/templates/*` as needed, or expand `markdown-page.js` to render section-specific layouts based on `context.id`.
...
- Build fails on Markdown page creation:
  - Check that `src/templates/markdown-page.js` exists and supports the queried fields.
